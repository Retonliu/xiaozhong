            ...
            <template #title>
                <van-row>
                    ...
                    <van-col class="toRight">
                        <van-icon name="arrow-down" @click="setI(user, index)" />
                        <van-action-sheet v-model="show" :actions="actions" @select="(item) => {onSelect(item)} " />
                    </van-col>
                </van-row>
  ...
  export default {
    ...
    methods: {
        ...
        storePost() {
            let user = this.comIndex.user;
            let data = {
                "userId": sessionStorage.userInfo.toString(),
                "vlogId": user.vlog.vlogId
            }
            this.$axios
            .post('http://47.115.90.9:8181/vlog/addStar', data)
            .then(response => {
                //console.info(response.data);
                if (response.data.code == 0) {
                    this.$toast("已加入收藏列表");
        ...
        delPost() {
            ...
            let urlData = 'http://47.115.90.9:8181/myVlog/deleteVlog/' + user.vlog.vlogId;
            ...
            .then(response => {
                //console.info(response.data);
                if (response.data.code == 0) {
                    this.dataAll.data.splice(index, 1);
                    this.$toast("删除成功");
                }
            });
        }
